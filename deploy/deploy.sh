...
helm upgrade \
    ...
    --set app.container.env.getStreamKey="${GET_STREAM_KEY}"\
    --set app.container.env.getStreamSecret="${GET_STREAM_SECRET}"\
    --set app.container.env.getStreamTokenExpiryDays="${GET_STREAM_TOKEN_EXPIRY_DAYS}"\
    ...
    --wait \
    --timeout 300s \
    -f ./charts/mycarehub/values.yaml \
    $APPNAME \
    ./charts/mycarehub
