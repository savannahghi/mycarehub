...
func Test_FilterFacilities(t *testing.T) {
	ctx := context.Background()
	graphQLURL := fmt.Sprintf("%s/%s", baseURL, "graphql")
	...
	graphqlQuery := `
	query filterFacilities($serviceID: String, $distance: Float, $locationInput: LocationInput!, $paginationInput: PaginationsInput!){
		filterFacilities(serviceID: $serviceID, distance: $distance, locationInput: $locationInput, paginationInput: $paginationInput){
		  facilities{
			id
			name
			phone
	...
	tests := []struct {
		...
		{
			name: "Sad case: unable to filter facilities",
			args: args{
				query: map[string]interface{}{
					"query": graphqlQuery,
					"variables": map[string]interface{}{
