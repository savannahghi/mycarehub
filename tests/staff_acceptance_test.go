...
func TestUpdateOrganisationAdminPermission(t *testing.T) {
	ctx := context.Background()
	graphQLURL := fmt.Sprintf("%s/%s", baseURL, "graphql")
	...
	graphqlMutation := `
	mutation updateOrganisationAdminPermission($staffID: String!, $isOrganisationAdmin: Boolean!){
		updateOrganisationAdminPermission(staffID: $staffID, isOrganisationAdmin: $isOrganisationAdmin)
	  }
	`
	...
	tests := []struct {
		...
		{
			name: "success: set organisation admin",
			args: args{
				query: map[string]interface{}{
					"query": graphqlMutation,
					"variables": map[string]interface{}{
...
func TestSearchStaffUser(t *testing.T) {
	ctx := context.Background()
	graphQLURL := fmt.Sprintf("%s/%s", baseURL, "graphql")
