...
func TestActivateOrDeactivateUser(t *testing.T) {
	ctx := context.Background()
	graphQLURL := fmt.Sprintf("%s/%s", baseURL, "graphql")
	...
	graphqlMutation := `
	mutation activateOrDeactivateUser($userID: ID!) {
		activateOrDeactivateUser(userID: $userID)
	  }
	`
	...
	tests := []struct {
		...
		{
			name: "valid: activate user - consumer",
			args: args{
				query: map[string]interface{}{
					"query": graphqlMutation,
					"variables": map[string]interface{}{
