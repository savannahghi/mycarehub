...
// SurveyInput represents the payload that is to be sent when a user is creating a questionnaire.
type QuestionnaireInput struct {
	...
	ValidMonth                 *int                            `json:"valid_month"`      // nullable
	HasExpiry                  bool                            `json:"has_expiry"`       // if validity is not set, this should be false, DONT include in input (add as a field in db)
	...
	FrequencyMonth             *int                            `json:"frequency_month"`  //If null, then it is a one time survey, applies to all other frequencies
	...
	QuestionnaireScreeningTool QuestionnaireScreeningToolInput `json:"screening_tool"`
...
type QuestionnaireScreeningToolInput struct {
	...
	ClientType []string `json:"client_type"` // Targets clients of the types provided, if empty, then send to all types
	Genders    []string `json:"genders"`     // Targets clients of the genders provided, if empty, then send to all genders
	AgeRange   []string `json:"age_range"`   // Targets clients of the age ranges provided, if empty, then send to all ages
}
...
type QuestionnaireQuestionInput struct {
	...
	QuestionType      string                             `json:"question_type"`       // the type of question, enum: Open ended or close ended
	ResponseTypeValue string                             `json:"response_type_value"` // the type of response expected for the question, enum: String, bool, number...
