...
func TestProgramInput_ParseProgram(t *testing.T) {
	type fields struct {
		ProgramIndex string
	}
	type args struct {
	...
	tests := []struct {
		...
		{
			name:   "Happy case: valid ProgramIndex number",
			fields: fields{ProgramIndex: "1"},
			args: args{
				programsLength: 2,
			},
			wantErr: false,
		...
		{
			name:   "Sad case: missing ProgramIndex number",
			fields: fields{},
			args: args{
				programsLength: 1,
			},
		...
		{
			name:   "Sad case: invalid input",
			fields: fields{ProgramIndex: "one"},
			args: args{
				programsLength: 1,
			},
			wantErr: true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			p := &CMDProgramInput{
				ProgramIndex: tt.fields.ProgramIndex,
			}
			got, err := p.ParseProgram(tt.args.programsLength)
			if (err != nil) != tt.wantErr {
				t.Errorf("ProgramInput.ParseProgram() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !tt.wantErr && got == nil {
				t.Errorf("ProgramInput.ParseProgram() = %v", got)
			}
		})
	}
}

func TestFacilityInput_ParseFacility(t *testing.T) {
	type fields struct {
		FacilityIndex string
	}
	type args struct {
		facilitiesLength int
	}
	tests := []struct {
		name    string
		fields  fields
		...
		{
			name:   "Happy case: valid FacilityIndex number",
			fields: fields{FacilityIndex: "1"},
			args: args{
				facilitiesLength: 2,
			},
			wantErr: false,
		...
		{
			name:   "Sad case: missing FacilityIndex number",
			fields: fields{},
			args: args{
				facilitiesLength: 1,
			},
		...
		{
			name:   "Sad case: invalid Input",
			fields: fields{FacilityIndex: "one"},
			args: args{
				facilitiesLength: 1,
			},
			wantErr: true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			f := &CMDFacilityInput{
				FacilityIndex: tt.fields.FacilityIndex,
			}
			got, err := f.ParseFacility(tt.args.facilitiesLength)
			if (err != nil) != tt.wantErr {
				t.Errorf("FacilityInput.ParseFacility() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !tt.wantErr && got == nil {
				t.Errorf("FacilityInput.ParseFacility() = %v", got)
			}
		})
	}
}
