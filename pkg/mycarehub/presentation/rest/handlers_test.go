...
func TestMyCareHubHandlersInterfacesImpl_UpdateProgram(t *testing.T) {
	emptyProgramPayload := dto.UpdateProgramInput{
		ProgramID:    "",
		FHIRTenantID: "",
	}
	marshalledEmptyPayload, err := json.Marshal(emptyProgramPayload)
	if err != nil {
		t.Errorf("failed to marshal payload")
		return
	}
	...
	tests := []struct {
	...
	}{
		{
			name: "Sad Case - Empty payload",
			args: args{
				url:        fmt.Sprintf("%s/internal/program", baseURL),
				httpMethod: http.MethodPost,
				body:       bytes.NewBuffer(marshalledEmptyPayload),
			},
			wantStatus: http.StatusBadRequest,
