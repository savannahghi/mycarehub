...
import (
	...
	fakeMatrix "github.com/savannahghi/mycarehub/pkg/mycarehub/infrastructure/services/matrix/mock"
...
func TestServicePubSubMessaging_NotifyRegisterMatrixUser(t *testing.T) {
	type args struct {
		ctx     context.Context
		payload *dto.MatrixUserRegistrationPayload
	}
	tests := []struct {
		name    string
		args    args
		...
		{
			name: "Happy case: register matrix user",
			args: args{
				ctx: nil,
				payload: &dto.MatrixUserRegistrationPayload{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.name == "Sad case: unable to register matrix user" {
				fakeExtension.MockPublishToPubsubFn = func(ctx context.Context, pubsubClient *pubsub.Client, topicID, environment, serviceName, version string, payload []byte) error {
					return errors.New("unable to publish to pubsub")
				}
			}

			if err := ps.NotifyRegisterMatrixUser(tt.args.ctx, tt.args.payload); (err != nil) != tt.wantErr {
				t.Errorf("ServicePubSubMessaging.NotifyRegisterMatrixUser() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
