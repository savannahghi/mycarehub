...
func (us *UseCasesUserImpl) Register(ctx context.Context, payload *dto.SignUpPayload, selfRegistered bool) (*dto.ClientRegistrationOutput, error) {
	...
	if selfRegistered {
		var (
			defaultOrganisationID = serverutils.MustGetEnvVar("DEFAULT_ORG_ID")
			defaultProgramID      = serverutils.MustGetEnvVar("DEFAULT_PROGRAM_ID")
			defaultMFLCode        = serverutils.MustGetEnvVar("DEFAULT_FACILITY_MFL_CODE")
		)
		...
		if !exists {
			return nil, fmt.Errorf("facility with MFLCode %s does not exist", defaultMFLCode)
		}
...
func (us *UseCasesUserImpl) SearchClientUser(ctx context.Context, searchParameter string) ([]*domain.ClientProfile, error) {
	...
	trimmedSearchParameter := strings.TrimSpace(searchParameter)

	clientProfile, err := us.Query.SearchClientProfile(ctx, trimmedSearchParameter)
	if err != nil {
		helpers.ReportErrorToSentry(err)
		return nil, fmt.Errorf("unable to get client profile: %w", err)
	}
...
func (us *UseCasesUserImpl) DeleteClientProfile(ctx context.Context, clientID string) (bool, error) {
	...
	clientProfiles, err := us.Query.GetUserClientProfiles(ctx, *clientProfile.User.ID)
	if err != nil {
		helpers.ReportErrorToSentry(err)
		return false, err
	}

	staffProfiles, err := us.Query.GetUserStaffProfiles(ctx, *clientProfile.User.ID)
	if err != nil {
		helpers.ReportErrorToSentry(err)
		return false, err
	}

	caregiverProfiles, err := us.Query.SearchCaregiverUser(ctx, clientProfile.User.Username)
	if err != nil {
		helpers.ReportErrorToSentry(err)
		return false, err
	}

	if len(clientProfiles) == 1 && len(staffProfiles) == 0 && len(caregiverProfiles) == 0 {
		userID = clientProfile.User.ID
	}

	phone, err := us.Query.GetContactByUserID(ctx, clientProfile.User.ID, "PHONE")
	if err != nil {
		helpers.ReportErrorToSentry(err)
		return false, exceptions.ContactNotFoundErr(err)
	}
	...
	if userID != nil {
		...
		matrixUserID := fmt.Sprintf("@%s:%s", clientProfile.User.Username, serverutils.MustGetEnvVar("MATRIX_DOMAIN"))
	...
	err = us.Delete.DeleteClientProfile(ctx, *clientProfile.ID, userID)
	if err != nil {
		helpers.ReportErrorToSentry(err)
