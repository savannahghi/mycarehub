...
func TestUseCasesUserImpl_AddCaregiverToClient(t *testing.T) {
	ID := gofakeit.UUID()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.name == "sad case: unable to add caregiver to client" {
				fakeDB.MockAddCaregiverToClientFn = func(ctx context.Context, clientCaregiver *domain.CaregiverClients) error {
			...
			got, err := us.AddCaregiverToClient(tt.args.ctx, tt.args.input)
			if (err != nil) != tt.wantErr {
