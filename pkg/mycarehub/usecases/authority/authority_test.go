...
func TestUsecaseAuthorityImpl_AssignOrRevokeRoles(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "happy case: successfully revoke roles to a user",
			args: args{
				ctx:    context.Background(),
				userID: uuid.New().String(),
				roles:  []enums.UserRoleType{enums.UserRoleTypeSystemAdministrator},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.name == "sad case: failed to assign roles to a user" {
				fakeDB.MockAssignOrRevokeRolesFn = func(ctx context.Context, userID string, roles []enums.UserRoleType, isAssigning bool) (bool, error) {
					return false, fmt.Errorf("failed to assign roles to a user")
				}
			}

			got, err := u.AssignOrRevokeRoles(tt.args.ctx, tt.args.userID, tt.args.roles, tt.args.isAssigning)
			if (err != nil) != tt.wantErr {
