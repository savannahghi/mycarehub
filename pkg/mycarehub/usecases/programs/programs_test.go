...
func TestUsecaseProgramsImpl_GetProgramByNameAndOrgName(t *testing.T) {
	fakeDB := pgMock.NewPostgresMock()
	fakeExtension := extensionMock.NewFakeExtension()
	fakeGetStream := getStreamMock.NewGetStreamServiceMock()
	fakePubsub := pubsubMock.NewPubsubServiceMock()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.name == "Sad case: failed to get program by name ang org id" {
				fakeDB.MockGetProgramByNameAndOrgNameFn = func(ctx context.Context, programName, organisationName string) (*domain.Program, error) {
					return nil, fmt.Errorf("an error occurred")
				}
			}

			_, err := u.GetProgramByNameAndOrgName(tt.args.ctx, tt.args.programName, tt.args.organisationName)
			if (err != nil) != tt.wantErr {
				t.Errorf("UsecaseProgramsImpl.GetProgramByNameAndOrgName() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
		})
	}
