...
// names borrowed from moby project https://github.com/moby/moby
// while this will suffice for now, lets add more african names and heroes
...
var (
	...
	right = [...]string{
		...
		// Makmende is a fictional Kenyan superhero character https://en.wikipedia.org/wiki/Makmende
		"makmende",
...
// formatted as "adjective_surname". For example '@focused_turing23254'. If retry is non-zero, a random
// integer between 0 and 10 will be added to the end of the name, e.g `focused_turing3`
func GetRandomName() *string {
	rand.Seed(time.Now().UnixNano())
	name := fmt.Sprintf("@%s_%s%v%v254", left[rand.Intn(len(left))], right[rand.Intn(len(right))], rand.Intn(len(left)), rand.Intn(len(right)))
	return &name
}
