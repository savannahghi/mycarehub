...
func TestValidateTokenPayload(t *testing.T) {
	ctx, tokenString := base.GetAuthenticatedContextAndBearerToken(t)
	tokenPayload := &UserAuthToken{
		Token: tokenString,
	}
	authToken, _ := base.ValidateBearerToken(ctx, tokenString)

	validTokenJSONBytes, err := json.Marshal(tokenPayload)
	assert.Nil(t, err)
	assert.NotNil(t, validTokenJSONBytes)
	goodRequest := httptest.NewRequest(http.MethodGet, "/", nil)
	goodRequest.Body = ioutil.NopCloser(bytes.NewReader(validTokenJSONBytes))

	emptyToken := &UserAuthToken{}
	emptyTokenJSONBytes, err := json.Marshal(emptyToken)
	assert.Nil(t, err)
	assert.NotNil(t, emptyTokenJSONBytes)
	badRequest := httptest.NewRequest(http.MethodGet, "/", nil)
	badRequest.Body = ioutil.NopCloser(bytes.NewReader(emptyTokenJSONBytes))
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := ValidateTokenPayload(tt.args.w, tt.args.r)
			if (err != nil) != tt.wantErr {
				t.Errorf("ValidateTokenPayload() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("ValidateTokenPayload() = %v, want %v", got, tt.want)
			}
		})
	}
}
