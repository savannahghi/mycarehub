...
func TestService_SetPresence(t *testing.T) {
	type args struct {
		ctx      context.Context
		presence bool
	}
	tests := []struct {
		...
		{
			name: "invalid case, no uid in context",
			args: args{
				ctx:      context.Background(),
				presence: true,
			},
		...
		{
			name: "valid case, uid in context",
			args: args{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := NewService()
			got, err := s.SetPresence(tt.args.ctx, tt.args.presence)
			if (err != nil) != tt.wantErr {
				t.Errorf("Service.SetPresence() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("Service.SetPresence() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func TestService_GetPresence(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := NewService()
			_, err := s.GetPresence(tt.args.ctx)
			if (err != nil) != tt.wantErr {
				t.Errorf("Service.GetPresence() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
		})
	}
